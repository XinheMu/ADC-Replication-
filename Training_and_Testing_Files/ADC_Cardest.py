...
def get_query_basics(target_rectangle):
    ...
    target_rectangle=target_rectangle-(target_rectangle-goto_value_one)*(target_rectangle>reg_const_zero[0:1,:])*(target_rectangle<reg_const_zero[1:2,:])-(deducted_value_one)*(target_rectangle>reg_const_zero[1:2,:])-(target_rectangle-goto_value_two)*(target_rectangle>reg_const_zero[3:4,:])*(target_rectangle<reg_const_zero[4:5,:])-(deducted_value_two)*(target_rectangle>reg_const_zero[4:5,:])
    ...
    volume=torch.prod(query_edge_length)
    minedge=torch.min(query_edge_length)
    if minedge<=0:
        print(target_rectangle)
        print(minedge)
        raise ValueError('minedge<0')
    return volume.item(),target_rectangle,attribute_not_covered   
...
def run(dname,uvar,dim_init,kernum,tm,workload_size,bayes_source_attributes=None,bayes_called_attributes=None,bayes_assist_attributes=None,nan_to=-1000000000,working_mode='ADC',output_mode='qerror',threshold=100):
    global dataset_name, unit_of_variables, dimension, dimension_init, indexes, kernels_num, tmin, evalnet_head, evalnet_tail, kernels_matrix, classifier, timeused, j, actualcard, minimum_rad, bayesnet
    ...
    for i in range(0,workload_size):
        ...
        if output_mode=='sel':
            ...
            e=actualcard[i]/orisize
        estimate_selectivity[i]=CardEst_Implement_Selective(torch.clone(target_rectangle),bayes_source_attributes,bayes_called_attributes,bayes_assist_attributes,nan_to,working_mode)/orisize
        if output_mode=='qerror':
        ...
        if output_mode=='qerror':
            if max(error[i],1/error[i])>threshold:
                ...
                print(get_query_basics(target_rectangle))
    ...
    if output_mode=='qerror':
        ...
        print("95th")
        print(SortQ[ma.ceil(workload_size*0.95)])
        print("99th")
        print(SortQ[ma.ceil(workload_size*0.99)])
        print("MAX")
...
if __name__ == "__main__":
    ...
    cutoff=43
    gmmker=1280
    params=['Estimator','higgs','ADC+','qerror',"[1e-3,1e-3,1e-3,1e-3,1e-3,1e-3,1e-3]",'7','1/1280','10000',"[]","[]","[]",'-10000000000','100','25']
    ...
    bsource_attributes=ast.literal_eval(params[8])
    bcalled_attributes=ast.literal_eval(params[9])
    bassist_attributes=ast.literal_eval(params[10])
    nanto=int(params[11])
    errorthreshold=int(params[12])
    draws=int(params[13])
